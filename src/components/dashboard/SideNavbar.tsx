...
function SidebarContent({ user, pathname, isMobile }: SidebarContentProps) {
  if (!user || user.role === "USER") return null;

  const [openSections, setOpenSections] = useState<Record<string, boolean>>({});
  const role = (user?.role as DashboardRole) ?? "SELLER";
...
export const DashboardSidebar = ({ initialUser }: DashboardSidebarProps) => {
...
export const MobileSideNav = ({ initialUser }: MobileSideNavProps) => {
  const { data: user } = useCurrentUserQuery(initialUser);
  const pathname = usePathname();

  return <SidebarContent user={user ?? null} pathname={pathname} isMobile />;
};
