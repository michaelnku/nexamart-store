...
type MarketplaceMediaUploaderProps = {
  value: JsonFile[];
...
export function MarketplaceMediaUploader({
  value,
  onChange,
  endpoint,
  onDelete,
  ...
  }, [helperText, maxImages, slotsRemaining]);
          ...
          {value.map((image, index) => {
            ...
            return (
              <div
                key={image.key}
                className={cn(
                  "group relative aspect-square overflow-hidden rounded-[24px] border border-slate-200 bg-slate-100 shadow-[0_18px_48px_-36px_rgba(15,23,42,0.35)] dark:border-zinc-800 dark:bg-zinc-900",
                  previewContainerClassName,
                )}
              >
                <Image
                  src={image.url}
                  alt={previewLabel(index)}
                  fill
                  className={cn(
                ...
                {onDelete ? (
                  <button
                    type="button"
                    aria-label={`Remove ${previewLabel(index)}`}
