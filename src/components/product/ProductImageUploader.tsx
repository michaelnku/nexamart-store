...
export function ProductImageUploader({
  ...
  const [enhancementItem, setEnhancementItem] = useState<EnhancementItem | null>(
    null,
  );
  ...
  const helperText = useMemo(() => {
    if (slotsRemaining === 0) return `You have reached the ${maxImages}-image limit.`;
  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        onKeyDown={(event) => {
          if ((event.key === "Enter" || event.key === " ") && !totalBusy && slotsRemaining > 0) {
        ...
        className={cn(
          "group relative overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(145deg,#ffffff_0%,#f8fbff_55%,#eef6ff_100%)] p-6 shadow-[0_24px_80px_-56px_rgba(15,23,42,0.45)] transition",
          isDragActive && "border-[#3c9ee0] shadow-[0_30px_90px_-48px_rgba(60,158,224,0.45)]",
          (totalBusy || slotsRemaining === 0) && "cursor-not-allowed opacity-80",
          !totalBusy && slotsRemaining > 0 && "cursor-pointer hover:border-[#3c9ee0]/50 hover:shadow-[0_32px_90px_-58px_rgba(60,158,224,0.38)]",
        )}
      >
        <input
          ref={inputRef}
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(60,158,224,0.12),transparent_38%),radial-gradient(circle_at_bottom_left,rgba(15,23,42,0.06),transparent_28%)]" />

        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#3c9ee0] text-white shadow-lg shadow-sky-200/70">
              {processing ? (
            ...
            <div className="space-y-1">
              <p className="text-xs font-medium uppercase tracking-[0.24em] text-[#3c9ee0]">
                Tip: Clean backgrounds improve product visibility and conversion.
              </p>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-semibold text-slate-950">
                  Product gallery
                </h3>
                <span className="rounded-full border border-sky-200 bg-white px-2.5 py-0.5 text-xs font-medium text-sky-700">
          ...
          <Button
            type="button"
            disabled={totalBusy || slotsRemaining === 0}
            className="rounded-2xl bg-[#3c9ee0] px-5 text-white hover:bg-[#2d8ac8]"
            onClick={(event) => {
        ...
        <div className="relative mt-6 rounded-[24px] border border-dashed border-slate-300 bg-white/85 p-8 text-center">
          <div className="mx-auto flex max-w-md flex-col items-center gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">
                JPG, PNG, WEBP and similar image formats supported. Large source files are optimized before upload.
              </p>
            </div>
            ...
            {slotsRemaining === 0 ? (
              <div className="mt-2 inline-flex items-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
        ...
        {currentItem ? (
          <div className="relative mt-4 rounded-2xl border border-sky-100 bg-sky-50/70 px-4 py-3 text-sm text-sky-900">
            Preparing <span className="font-medium">{currentItem.file.name}</span>
            {queue.length > 0 ? ` and ${queue.length} more queued image${queue.length > 1 ? "s" : ""}.` : "."}
          </div>
        ) : null}
        {enhancementItem ? (
          <div className="relative mt-4 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
            Review the cropped image and optionally run AI Clean Background before upload.
          </div>
        ) : null}
      </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
          {value.map((image, index) => {
            ...
            return (
              <div
                key={image.key}
                className="group relative aspect-square overflow-hidden rounded-[24px] border border-slate-200 bg-slate-100 shadow-[0_18px_48px_-36px_rgba(15,23,42,0.35)]"
              >
                <Image
                  src={image.url}
                ...
                {onDelete ? (
                  <button
                    ...
                    className="absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-slate-900 shadow-lg transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {isDeleting ? (
