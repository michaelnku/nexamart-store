...
import { AiReviewQueueFilters } from "./_components/AiReviewQueueFilters";
import { AiReviewQueueTable } from "./_components/AiReviewQueueTable";
...
function buildAiPageHref(
  filters: ReturnType<typeof parseAiModerationSearchParams>,
  page: number,
) {
...
export default async function ModeratorAiReviewQueuePage(props: {
  ...
  const pageStart =
    incidentsResult.pagination.totalItems === 0
      ? 0
      : (incidentsResult.pagination.page - 1) * AI_MODERATION_PAGE_SIZE + 1;
  const pageEnd =
    incidentsResult.pagination.totalItems === 0
      ? 0
      : Math.min(
          incidentsResult.pagination.page * AI_MODERATION_PAGE_SIZE,
  ...
  return (
    <div className="space-y-6 text-slate-950 dark:text-zinc-100">
      ...
      <AiReviewQueueTable incidents={incidentsResult.items} />
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_22px_60px_-40px_rgba(15,23,42,0.2)] dark:border-zinc-800 dark:bg-zinc-950 md:flex-row md:items-center md:justify-between">
        <div className="text-sm text-slate-500 dark:text-zinc-400">
          ...
          {incidentsResult.pagination.totalItems.toLocaleString()} matching AI
          incidents
        </div>
        ...
        <Pagination className="justify-start md:justify-end">
          <PaginationContent>
            ...
            {visiblePages[0] !== 1 ? (
              <PaginationItem>
                <PaginationLink href={buildAiPageHref(filters, 1)}>
                  1
                </PaginationLink>
              </PaginationItem>
            ) : null}
            ...
            {visiblePages.map((page) => (
              <PaginationItem key={page}>
                <PaginationLink
                  href={buildAiPageHref(filters, page)}
