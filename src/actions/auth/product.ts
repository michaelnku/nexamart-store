...
import {
  createFoodProductRelationsInTx,
...
export const createProductAction = async (values: productSchemaType) => {
  try {
    ...
    const productImageAssets = images.length
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (productImageAssets.length) {
        await tx.productImage.createMany({
          data: productImageAssets.map((asset) => ({
      ...
      if (isFoodStore) {
        await createFoodProductRelationsInTx(tx, createdProduct.id, {
          foodConfig,
          foodOptionGroups,
        });
      }
